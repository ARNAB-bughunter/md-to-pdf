...
def converter(markdown_text: str):
    ...
    <html>
      ...
      <body>
        {clean_html}
        <div class="footer">Generated by <a href="https://markdowntopdf.cloud">markdowntopdf.cloud</a></div>
      </body>
    </html>
    """
    
    # Build CSS with conditional underline and footer
    css_string = f"""
        @page {{
            margin-bottom: 1.5cm;
        }}
        * {{margin: 0; padding: 0;}}
        body {{font-family: Helvetica, Arial, sans-serif; font-size: 10pt; line-height: 1.6; position: relative; min-height: 100vh;}}
        .footer {{
            position: fixed;
            bottom: 0;
            left: 0;
